...
for p = 20:38
    if p < 20
        ...
        filename = [pname '_tt_s_'];
    else
        pname = dzname{p-19,1};
        filename = [pname '_dz_s_'];
    end
    for fre = 1:2
        ...
        for ch = 1:size(source.trial,1)
            fprintf('calculating phase %d/%d\n', ch, size(source.trial,1));
            source_complex(ch,:,:) = hilbert(squeeze(source.trial(ch, :, :)));
%             Ang_source(ch, :, :) = angle(source_complex(ch,:,:));
        end
        %% calculating phase-based brain network
        
        % --- PLV - phase locking value ---
        [PLV.fullconn,PLI.fullconn,wPLI.fullcon] = ...
            calphasesyn(source_complex);
        ...
        PLV.chpath = charpath(plvD_temp);
        
        pliD_temp = distance_bin(PLI.conn_thre_b);
        PLI.chpath = charpath(pliD_temp);
        
        wpliD_temp = distance_bin(wPLI.conn_thre_b);
        wPLI.chpath = charpath(wpliD_temp);
        ...
        clear source source_complex  PLV PLI wPLI
